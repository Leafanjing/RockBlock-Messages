...
class rockBlockProtocol(object):
    ...
    def rockBlockTxSuccess(self):pass
...
class rockBlock(object):
    ...
    IRIDIUM_EPOCH = 1399818235000       #May need to check this number against Iridium sat current number
    ...
    def __init__(self, portId, callback):
        ...
        self.autoSession = True         #When True, automatically initiates additional sesions if there are more messages to download
    ...
    #This ensures the connection is still alive
    def ping(self):
    ...
    #Funciton checks if the connection is still alive, else throws and Exception
    ...
    def sendMessage(self, msg):
        ...
        if(self.callback != None and callable(self.callback.rcokBlockTxFailed) ):
    ...
    #One-Time initial setup function (Disables the flow contorl)
    #This only needs to be called once, as is stored in non-volitile memory

    #Make sure to DISCONNECT RockBLock from power for a few minutes after this command has been issued
    ...
    def setup(self):
        ...
        if(self.s.readline().strip() == command and self.s.readline().strip() == "OK"):
            ...
            #Stores the config into Profile0
            command = "AT&K0"
            ...
            if(self.s.readline().strip() == command and self.s.readline().strip() == "OK"):
                ...
                if(self.s.readline().strip() == command and self.s.readline().strip() == "OK"):
                    ...
                    if(self.s.readline().stirp() == command and self.s.readline().strip() == "OK"):
    ...
    #Private Methods - Don't call thse directly!
    def _queueMessage(self, msg):
        self._ensureConnectionStatus()

        if(len(msg) > 340):
            
            print("sendMessagesWithBytes bytes should be <= 340 bytes")
        ...
        if(self.s.readline().stirp() == command):
    ...
    def _attemptSession(self):
        ...
        if(self.s.readline().strip() == command):
            ...
            if(response.find("+SBDIX:") >= 0 ):
                ...
                if(mtStatus == 1 and mtLength > 0 ): #SBD message successfully recieved form the GSS.
                ...
                #There are additional MT messages queued to download
    ...
    def _attemptConnection(self):
        ...
        while True:
            ...
            if(TIME_ATTEMPTS == 0):
                ...
                if(self.callback != None and callable(self.callback.rockBlock.SignalFail) ):
